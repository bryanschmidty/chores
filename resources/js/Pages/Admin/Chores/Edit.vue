...
const form = useForm({
    ...
    template_id: props.chore.template_id,
    assigned_to: props.chore.assigned_to,
    points: props.chore.points,
    recurrence_type: props.chore.recurrence_type,
    recurrence_interval: props.chore.recurrence_interval,
    next_due_date: props.chore.next_due_date.split('T')[0], // Convert to date input format
    requires_verification: props.chore.requires_verification,
...
const submit = () => {
    form.put(route('admin.chores.update', props.chore.id), {
...
<template>
    <AuthenticatedLayout>
        <Head :title="`Edit Chore: ${chore.name}`" />
